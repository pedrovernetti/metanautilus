...
class ColumnExtension(GObject.GObject, Nautilus.ColumnProvider, Nautilus.InfoProvider):
	...
	def info_fetching_failure(self, src, filename):
		if src != "": src = src + " "
		print ("\033[1mmetadata-on-nautilus\033[0m :: Could not fetch ", "")
		print (src + " information from: " + filename, "\n\n")
	...
	def metadata_fetching_failure(self, fmt, filename):
		...
		print ("\033[1mmetadata-on-nautilus\033[0m :: Could not fetch ", "")
		print (fmt + "metadata from:" + filename, "\n\n")
	...
	def fetch_asf_info(self, file, filename):
		...
		try:
			...
			file.add_string_attribute('bitrate', str(asfinfo.bitrate/1000) + kbps)
			file.add_string_attribute('samplerate', str(asfinfo.sample_rate) + Hz)
			file.add_string_attribute('duration', hhmmss(asfinfo.length))
			avfile.close()
		except:
			try: avfile.close()
			except: pass
	...
	def fetch_media_info(self, file, filename, filemime):
		if filemime.endswith(("matroska", "flv", "mpeg", "wav")):
			try:
				...
				if not fetched: self.metadata_fetching_failure("some", filename)
			except:
				self.metadata_fetching_failure("", filename)
	...
	def fetch_audio_info(self, file, filename, filemime):
		...
		self.fetched = False;
		if filesig == b'ID3': read_id3_tags(file, filename)
		elif filetail == b'APETAGEX': read_ape_tags(file, filename)
		elif filesig == b'fLa': read_flac_tags(file, filename)
		...
		if filemime.endswith(('/mpeg', 'mp2')): # audio/mpeg, audio/x-mp2
			...
			file.add_string_attribute('bitrate', str(info.bitrate/1000) + kbps)
		elif filemime.endswith('aiff'): # audio/x-aiff
